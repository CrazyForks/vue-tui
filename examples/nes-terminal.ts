/**
 * Terminal NES emulator — CLI runner.
 *
 * Interactive:  bun run run:nes:terminal
 * Smoke:        VT_SMOKE=1 tsx examples/nes-terminal.ts
 *
 * Loads a NES ROM (default: the bundled MIT homebrew "Falling") and streams its
 * 256×240 frames into the terminal graphics protocol (kitty / iTerm2 / sixel)
 * via TVideo. Keyboard maps to the NES controller:
 *
 *   ←↑↓→ / WASD  → D-pad
 *   Z / J         → B (shoot)
 *   X / K         → A (jump)
 ...
 *   Q / Ctrl+C    → quit
 *
 * Run your own legally-owned ROM:
...
import {
  ...
  NES_VISIBLE_W,
  type NesButton,
} from "./nes/nes-video-game.js";
import { encodeRgbaPng } from "./nes/png.js";
import {
  copyPngToClipboard,
  getLeaderboard,
  listRoms,
...
const defaultRom = fileURLToPath(new URL("./nes/roms/falling.nes", import.meta.url));
const contraRom = fileURLToPath(new URL("./nes/roms/contra.nes", import.meta.url));
const romsDir = fileURLToPath(new URL("./nes/roms", import.meta.url));

/**
 * ROM resolution:
 *   1. VUE_TUI_NES_ROM env override (error if missing)
 *   2. VUE_TUI_NES_RANDOM=1 → pick a random .nes from the roms dir
 *   3. user-provided contra.nes
 *   4. bundled homebrew falling.nes
 */
let romPath: string;
if (process.env.VUE_TUI_NES_ROM) {
  ...
  if (candidates.length === 0) {
    process.stderr.write("[nes] VUE_TUI_NES_RANDOM=1 but no .nes files in the roms dir\n");
    process.exit(2);
  }
  romPath = candidates[Math.floor(Math.random() * candidates.length)]!;
} else {
  romPath = existsSync(contraRom) ? contraRom : defaultRom;
}

const romBytes = readFileSync(romPath);
if (romBytes.length < 16 || romBytes[0] !== 0x4e || romBytes[1] !== 0x45 || romBytes[2] !== 0x53 || romBytes[3] !== 0x1a) {
...
if (!smoke) {
  ...
  if (!info.mapperSupported) {
    process.stderr.write(
      `[nes] warning: this mapper is not supported by the bundled jsnes core; ` +
        `the game may fail to run.\n`,
    );
  }
  if (romPath === contraRom) {
    process.stderr.write(
      `[nes] playing your ROM at ${contraRom}. To play a different (legally owned) ROM, ` +
        `place it at examples/nes/roms/contra.nes or set VUE_TUI_NES_ROM.\n`,
    );
  } else if (romPath === defaultRom) {
    process.stderr.write(
      `[nes] playing the bundled homebrew (${defaultRom}) — this only verifies the pipeline. ` +
        `To play your own legally-owned game (e.g. an original Contra cartridge you own), ` +
        `place its .nes at examples/nes/roms/contra.nes and rerun:\n` +
        `      bun run run:nes:terminal\n` +
        `      (verify first with: bun run check:nes:rom)\n`,
    );
  }
  // Leaderboard teaser + share hint.
...
if (smoke) {
  ...
  let painted = 0;
  for (let i = 0; i < rgba.length; i += 4) if (rgba[i]! !== 0 || rgba[i + 1]! !== 0 || rgba[i + 2]! !== 0) painted++;
 ...
 * We therefore treat every keydown as "pressed now", remember the last keydown
 * time per NES button, and auto-release any button that has not been refreshed
 * for a while. Direction buttons are mutually exclusive (left/right, up/down)
 * so the last-pressed direction wins, exactly like a real D-pad.
 */
...
function handleShare(): void {
  ...
  const result = shareToX({
    ...
    postfix: "Beat me on the vue-tui leaderboard!",
  });

  // 1) image → clipboard, 2) caption → clipboard, 3) open X composer.
