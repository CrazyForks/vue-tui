...
暂停菜单提供继续、分享到 X、重开和退出。加载其他 ROM 时仍会映射完整 NES
方向键，并支持 `Z/J` = B、`X/K` = A、`Enter` = Start、`Shift` = Select。

> 模式选择的每个上下键事件会进入 NES 帧队列，并在两次按压间插入一个松开帧，
> 因此快速连续按下或下后立即按上都不会被 ROM 的按键锁存吞掉。
