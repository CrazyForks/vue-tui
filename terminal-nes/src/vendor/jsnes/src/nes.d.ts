...
export class NES {
  ...
  /** PPU instance — exposes frame pixel buffers (`.buffer: Uint32Array`). */
  ppu: {
    buffer: Uint32Array;
    [key: string]: unknown;
  };
  /** Controller state (1 and 2) for direct button reads in tests. */
  controllers: Record<1 | 2, import("./controller.js").Controller>;
  rom?: {
    mapperType?: number;
    romCount?: number;
    vromCount?: number;
